# -*- coding: utf-8 -*-
"""
Created on Sat Jun  5 14:46:45 2021

@author: leofilgueira
"""
# Modelagem de absorvedor sobre superfície rígida com inidência normal
# Comparação entre os modelos JCA e Delany-Bazley
...
sig  = 23000                        # [Rayl] ou [Ns/m^4] Resistividade ao fluxo
d    = 0.005                        # [m] Espessura do material poroso
poros = 0.83                        # [-] Porosidade
tortuos = 1.01                       # [-] Tortuosidade
compvc = 30                       # [um] Comprimento viscoso característico

Zc1, kc1 = delany_bazley(f, sig, c0, rho0)
Zc2, kc2 = allard_champoux(rho0, tortuos, sig, poros, compvc, viscos, omega, prandtl, P0)

# Impedância de superfície 
Zs = np.empty((Zc1.size, 2), dtype="complex64")

    
Zs[:,0] = -1j * (Zc1 / np.tan(kc1 * d))     # Dellany-Bazley

Zs[:,1] = -1j * (Zc2 / np.tan(kc2 * d))     # Johnson-Champoux-Allard
...
# Legendas
legendas = ['Delany-Bazley','Jonhson-Champoux-Allard']

# Gerando gráfico
plt.title("\u03A6=0.96 [-], \u03C3=25000 [Rayl], d=25 [mm], $\u03B1_\u221E$=1.1 [-], \u039B=100 [\u03BCm]")
...
np.savetxt("Zs_feltro.txt", Zs[:,0])
pd.DataFrame(Zs[:,0]).to_csv('Zs_feltro.csv',index_label = "Index")  
