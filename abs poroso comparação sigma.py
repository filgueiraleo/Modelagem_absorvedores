...
sig1  = 25000
sig2 = 50000
sig3 = 100000                      # [Rayl] ou [Ns/m^4] Resistividade ao fluxo
...
Zc1, kc1 = allard_champoux(rho0, tortuos, sig1, poros, compvc, viscos, omega, prandtl, P0)
Zc2, kc2 = allard_champoux(rho0, tortuos, sig2, poros, compvc, viscos, omega, prandtl, P0)
Zc3, kc3 = allard_champoux(rho0, tortuos, sig3, poros, compvc, viscos, omega, prandtl, P0)

Zs = np.empty((Zc1.size, 3), dtype="complex64")

Zs[:,0] = -1j * (Zc1/(np.tan(kc1*d)))   # Impedância de Superfície
Zs[:,1] = -1j * (Zc2/(np.tan(kc2*d)))
Zs[:,2] = -1j * (Zc3/(np.tan(kc3*d)))
...
# Legendas
legendas = ['\u03C3 = 25000 [Rayl]','\u03C3 = 50000 [Rayl]', '\u03C3 = 100000 [Rayl]']

# Gerando gráfico
plt.title("\u03A6=0.96 [-], d=4.0 [cm], \u03B1\u221E=1.1 [-], \u039B=100 [\u03BCm]")
